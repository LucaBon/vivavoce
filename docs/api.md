...
| `lang` | string | `"it"` | `it`, `en`, `es`, `fr` or `de`. The language the sentence is *in*, and the language the answer comes back in. Anything else falls back to Italian. The list is not prose: it is `engine/catalogs/`, and a test holds this row to it. |
